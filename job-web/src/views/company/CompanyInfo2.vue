<template>
  <div>
    <r-form ref="myForm1" :rules="rules" :form="form" :items="items" :save="save"></r-form>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      items: [
        ...
        {type: 'text', label: '用户名', prop: 'account', name: 'account', placeholder: '请更改用户名', options: []},
        {type: 'text', label: '密码', prop: 'password', name: 'password', placeholder: '请更改密码', options: []},
  ...
  methods: {
    ...
    save() {
      let flag = this.$refs['myForm1'].validateForm();
      if (flag) {
        let param = this.form;
        if (this.form.single) {
          param.single = 'Y';
